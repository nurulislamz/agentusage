...
const (
	LaunchdDaemonLabel = "com.agentusage.telemetryd"
	SystemdDaemonUnit  = "agentusage-telemetry.service"
	...
	WindowsScheduledTask = "agentUsage Telemetry Daemon"
)
...
func (m ServiceManager) InstallHint() string {
	return "agentusage telemetry daemon install"
}
...
func ServiceStatus(ctx context.Context, socketPath string, details bool) error {
	...
	if healthErr != nil {
		...
		if !installed {
			fmt.Println("  1. Install the service: ./bin/agentusage telemetry daemon install")
		} else {
			fmt.Println("  1. Reinstall to refresh the unit and restart: ./bin/agentusage telemetry daemon install")
		}
...
func writeServiceEnvFile(path string, env map[string]string) error {
	...
	b.WriteString("# Generated by `agentusage telemetry daemon install`.\n")
