...
//
// It also probes the exact Zen goUsage endpoint directly, because the
// provider discards its status (`if err == nil` — no diagnostic when the
// exact source is unavailable) and the dashboard then only shows the
// console scrape, whose rounded/staler numbers are the "off by a little
// bit" readings.
func TestDiagnostic_OpenCodeLiveUsageDelta(t *testing.T) {
	...
	// Direct probe: why would the exact source be unavailable?
	var probe goUsageResponse
	statusCode, _, probeErr := shared.FetchJSON(context.Background(),
		defaultBaseURL+goUsagePath, apiKey, &probe, New().Client())
	t.Logf("zen goUsage probe: HTTP %d err=%v", statusCode, probeErr)
	if probeErr != nil {
		t.Logf("DIAGNOSTIC: exact Zen API source unavailable (HTTP %d). "+
			"The provider skips it silently, so the dashboard shows "+
			"console-scraped values only — the likely source of the drift.",
			statusCode)
	}
	...
	logLiveSnapshot(t, "combined", snap)
	...
	logLiveSnapshot(t, "api-only", consoleFree)
	...
	for _, key := range []string{"rolling_usage", "weekly_usage", "monthly_usage_pct"} {
		...
		if !okC || !okA || combined.Used == nil || apiOnly.Used == nil {
			t.Logf("window %q: missing (combined=%v apiOnly=%v)", key, okC, okA)
			continue
		}
...
// logLiveSnapshot prints the snapshot's status and every metric present so
// the live-compare test explains itself when a source is unavailable.
func logLiveSnapshot(t *testing.T, label string, snap core.UsageSnapshot) {
	t.Helper()
	t.Logf("%s fetch: status=%s msg=%q scope=%q diagnostics=%v",
		label, snap.Status, snap.Message, snap.Attributes["auth_scope"], snap.Diagnostics)
	if len(snap.Metrics) == 0 {
		t.Logf("%s fetch: no metrics at all", label)
		return
	}
	for k, m := range snap.Metrics {
		if m.Used == nil {
			t.Logf("%s metric %s: no Used value", label, k)
			continue
		}
		t.Logf("%s metric %s: %.4f %s", label, k, *m.Used, m.Unit)
	}
}
